...
def CalcLike_grid(mx, expts, Ngrid = 100, maj = False, refine=False):
    ...
    if (refine):
        ...
        if (maj):
            (cp1, cn1, f1)  = CalcLike_refine(mx, expts, Ngrid+1, cpmax_maj_minus, cnmax_maj_minus, -1.0, delta0, maj=True)[1:]
            reflike_maj_minus  = CalcLike_refine(mx, expts, Ngrid+1, cp1, cn1, f1, delta1, maj=True)[0]
        
            (cp1, cn1, f1) = CalcLike_refine(mx, expts, Ngrid+1, cpmax_maj_plus, cnmax_maj_plus, 1.0, delta0, maj=True)[1:]
            ...
            majtest = CalcLike_refine(mx, expts, Ngrid+1, 2e-8, 1.5e-8, -1.0, 0.5, maj=True)[0]
            ...
            (cp1, cn1, f1) = CalcLike_refine(mx, expts, Ngrid+1, cpmax_dir, cnmax_dir, fmax_dir, delta0, maj=False)[1:]
